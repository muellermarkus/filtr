...
Filtr.comparators = {
  ...
  , $eq: function (a, b) {
    if (a instanceof Array) {
      console.log("$eq");
      return a.some(item => Filtr.comparators.$eq(item, b));
    }
    console.log(a,b);
    if (
      'object' === typeof b &&
      Object.keys(b).includes('$regex')
    ) {
      console.log('HHHEEEY');
      console.log(b,a);
      console.log((new RegExp(b['$regex'].source, 'i')).test(a));
      return (new RegExp(b['$regex'].source, 'i')).test(a);
    }
    console.log("nop");
    return a == b;
  }
  ...
  , $ne: function (a, b) {
    if (a instanceof Array) {
      return a.every(item => Filtr.comparators.$ne(item, b));
    }
    return a != b;
  }
 ...
 * @returns {Object|Array|Undefined} value
 */
      ...
      if ('undefined' !== typeof part.p) {
        if (tmp instanceof Array) {
          for (var j = 0; j < tmp.length; j++) {
            res = tmp
              .map(item => getPathValue(parsed.slice(i, l), item))
...
function parseQuery (query) {
  var stack = [];
  // Convert the query from an implicit $and into an explicit $and
  // in case one of the keys is a comparator
  // (because this caused problems with qry.test[0].fn being undefined)
  const comparators = Object.keys(Filtr.comparators);
  const queryKeys = Object.keys(query);
  if (
    queryKeys.length > 1 &&
    queryKeys.some(key => comparators.includes(key))
    ) {
    query = { $and: Object.keys(query).map((key) => { return { [key]: query[key] }; }) };
  }
  ...
  for (var cmd in query) {
    ...
    if (cmd[0] == '$') {
      ...
      if ('object' === typeof params
        && Object.keys(params).includes('$regex')) {
        qry.test = [{ fn: Filtr.comparators.$eq, params}]
        qry.path = parsePath(cmd);
      } else if ('string' == typeof params
      || 'number' == typeof params
      || 'boolean' == typeof params) {
        qry.test = parseFilter({ $eq: _maybeTransformToDate(params) });
        qry.path = parsePath(cmd);
      } else {
...
function parseFilter (query) {
  ...
  for (var test in query) {
    ...
    stack.push({
        fn: fn
      , params: traverse ? st : _maybeTransformToDate(params)
      , traverse: traverse
    });
  }
  return stack;
...
function _maybeTransformToDate(params) {
  if ('string' === typeof params) {
    var maybeDate = moment(params, 'YYYY-MM-DDTHH:mm:ss.SSSZ', true);
